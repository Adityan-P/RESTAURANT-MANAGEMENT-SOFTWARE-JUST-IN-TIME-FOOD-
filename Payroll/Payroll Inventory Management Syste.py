...
Pay_Inv = Tk()
Pay_Inv.geometry("1500x900")
Pay_Inv.resizable(0, 0)
Pay_Inv.title("Payroll  Management System")
...
m=[]
b=[]
details={"NAME":k,"ADDRESS":h,"EMPLOYER":j,"ID NUMBER":s,"HOURS WORKED": l,"NET PAYABLE":y,"WAGES PER HOUR":q,"TAX PAID":m,"GROSS SALARY":b}
def store():
    k.append("darshan ")
    j.append("adityan ")
    h.append("bennett university")
    s.append("112")
    l.append("60")
    y.append("48000")
    q.append("1000")
...
def WeeklySalary():
    ...
    if(HoursWorksPerWeek>40):
       ...
       paydue = (40 * WagesPerHours)+(HoursWorksPerWeek-40)*1.5*WagesPerHours
    else:
        paydue=HoursWorksPerWeek* WagesPerHours

    PaymnetDue = "Rs.", str(format(paydue, '.2f'))
    GrossPay.set(PaymnetDue)

    tax = paydue * 0.2
    TaxAbles = "Rs.", str(format(tax, '.2f'))
    Tax.set(TaxAbles)

    netPay = paydue - tax
    NetPays = "Rs.", str(format(netPay, '.2f'))
    NetPay.set(NetPays)
    ...
    if HoursWorksPerWeek > 40:
        ...
        OverTimeHour = "Rs.", str(format(OverTimeHours, '.2f'))
...
def ViewPayslip():
    ...
    txtPaySlip.insert(END, "IDNumber:\t\t" + NINumber.get() + "\n\n")
    ...
    txtPaySlip.insert(END, "Gross salary:\t\t" + GrossPay.get() + "\n\n")
#---Variable
Name = StringVar()
Employer = StringVar()
...
lblInfo = Label(Tops, font=('arial', 52, 'bold'), text="    Payroll  Management System      ",
                bd=10, anchor='w')
...
lblNINumber = Label(f1a, font=('arial', 20, 'bold'), text="ID Number",
                fg='black', bd=10, anchor='w')
...
btnstore = Button(f1b, padx=16, pady=16, bd=8, fg="black", font=('arial', 16, 'bold'), width=16,
                        height=1, text="Store Data", bg='white', command=store()).grid(row=0, column=3)
